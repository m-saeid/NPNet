...
def load_data_modelnet_r(dataset_dir, partition):

    all_data = []
    all_label = []
    for h5_name in glob.glob(os.path.join(dataset_dir, "modelnetR_ply_hdf5_2048", "ply_data_%s*.h5" % partition)):
...
if __name__ == "__main__":
    ...
    train = ModelNetR(modelnet_dir, 1024)
    test = ModelNetR(modelnet_dir, 1024, "test")

    from torch.utils.data import DataLoader

    train_loader = DataLoader(
        ModelNetR(modelnet_dir, partition="train", num_points=1024),
        num_workers=4,
        batch_size=32,
        shuffle=True,
        drop_last=True,
    ...
    train_set = ModelNetR(modelnet_dir, partition="train", num_points=1024)
    test_set = ModelNetR(modelnet_dir, partition="test", num_points=1024)
