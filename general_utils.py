import os
import csv
import random
import argparse

import numpy as np
from tqdm import tqdm

import torch
import torch.nn.functional as F
from torch.utils.data import DataLoader

from data.modelnet40 import ModelNet40
from data.modelnet_r import ModelNetR
from data.modelnet40fewshot import ModelNet40FewShot


def setup_parser():
    parser = argparse.ArgumentParser(description="Training script for point cloud classification")

    # Existing arguments...
    parser.add_argument("--model", type=str, default="npnet", help="Model type")
    parser.add_argument("--task", type=str, default="cls", help="Task (e.g., classification)")
    parser.add_argument("--dataset", type=str, default="modelnet40", choices=["modelnet40", "modelnet-R", "modelnet40fewshot"], help="Dataset to use")

    parser.add_argument("--batch_size", type=int, default=10, help="Batch size")
    parser.add_argument("--num_points", type=int, default=1024, help="Number of points per sample")
    parser.add_argument("--stages", type=int, default=4, help="Number of stages")
    parser.add_argument("--k", type=int, default=110, help="Number of neighbors for kNN")

    parser.add_argument("--init_dim", type=int, default=35, help="Initial embedding dimension")
    parser.add_argument("--stage_dim", type=int, default=35, help="Dimension after each stage (feature dim)")
    parser.add_argument("--baseline", type=float, default=0.1, help="baseline value used by embedding function") #
    parser.add_argument("--scaling", type=float, default=10.0, help="scaling value used by embedding function")  #
    parser.add_argument("--eps", type=float, default=1e-6, help="Epsilon value for numerical stability")         #

    # Few-Shot Learning
    parser.add_argument("--n_way", type=int, default=10, choices=[5, 10], help="Number of classes per episode")
    parser.add_argument("--k_shots", type=int, default=20, choices=[10, 20], help="Number of samples per class")

    parser.add_argument("--evaluation", action="store_true", default=False, help="Evaluation [True/False]")

    parser.add_argument("--fixed_sigma", type=float, default=None, help="Disable the adaptive_sigma mode of the embedding module and assign a fixed sigma value. If None, adaptive mode is used.")
    parser.add_argument("--fixed_blend", type=float, default=None, help="Disable the adaptive_blend mode of the embedding module and assign a fixed blend value. If None, adaptive mode is used")

    return parser


def additional_dataset_arguments(parser, dataset, way, shot):
    if dataset == "modelnet40":
        parser.add_argument("--feat_normalize", action="store_true", default=True, help="Apply feature normalization")
        parser.add_argument("--sigma", type=float, default=0.232, help="sigma value used by embedding function")
        parser.add_argument("--seed", type=int, default=42, help="Random seed")
    elif dataset == "modelnet-R":
        parser.add_argument("--feat_normalize", action="store_true", default=True, help="Apply feature normalization")
        parser.add_argument("--sigma", type=float, default=0.274, help="sigma value used by embedding function")
        parser.add_argument("--seed", type=int, default=42, help="Random seed")
    elif dataset == "modelnet40fewshot":
        parser.add_argument("--feat_normalize", action="store_true", default=True, help="Apply feature normalization")
        if way == 5:
            if shot == 10:
                parser.add_argument("--sigma", type=float, default=0.282, help="sigma value used by embedding function")
                parser.add_argument("--seed", type=int, default=852, help="Random seed")
            elif shot == 20:
                parser.add_argument("--sigma", type=float, default=0.367, help="sigma value used by embedding function")
                parser.add_argument("--seed", type=int, default=537, help="Random seed")
        elif way == 10:
            if shot == 10:
                parser.add_argument("--sigma", type=float, default=0.205, help="sigma value used by embedding function")
                parser.add_argument("--seed", type=int, default=68, help="Random seed")
            elif shot == 20:
                parser.add_argument("--sigma", type=float, default=0.228, help="sigma value used by embedding function")
                parser.add_argument("--seed", type=int, default=479, help="Random seed")

def get_arguments():
    parser = setup_parser()
    args, _ = parser.parse_known_args()
    additional_dataset_arguments(parser, args.dataset, args.n_way, args.k_shots)
    args = parser.parse_args()
    args.csv_file = generate_csv_filename(args)
    return args


def generate_csv_filename(args):
    csv_file = f"{args.model}_{args.task}_{args.dataset}_n{args.feat_normalize}"
    if args.dataset == "modelnet40fewshot":
        csv_file += f"_n{args.n_way}_k{args.k_shots}"
    return os.path.join("./eval", csv_file + ".csv")


def get_dataloader(dataset, num_points, batch_size, partition, **kwargs):
    num_workers = 8
    return DataLoader(
        dataset(num_points=num_points, partition=partition, **kwargs),
        num_workers=num_workers,
        batch_size=batch_size,
        shuffle=True,
        drop_last=True,
    )


def get_cls_dataloader(dataset_dir, args):
    if args.dataset == "modelnet40":
        # dataset_dir = os.path.join(dataset_dir, "modelnet40")
        dataset = ModelNet40
        kwargs = {}
    elif args.dataset == "modelnet-R":
        # dataset_dir = os.path.join(dataset_dir, "modelnet-R")
        dataset = ModelNetR
        kwargs = {}
    elif args.dataset == "modelnet40fewshot":
        # dataset_dir = os.path.join(dataset_dir, "modelnet_fewshot")
        dataset = ModelNet40FewShot
        kwargs = {"n_way": args.n_way, "k_shot": args.k_shots}
    else:
        raise ValueError("Unsupported dataset")

    train_loader = get_dataloader(
        dataset,
        args.num_points,
        args.batch_size,
        "train",
        dataset_dir=dataset_dir,
        **kwargs,
    )
    test_loader = get_dataloader(
        dataset,
        args.num_points,
        args.batch_size,
        "test",
        dataset_dir=dataset_dir,
        **kwargs,
    )

    return train_loader, test_loader


def load_csv_as_dict(csv_file):
    """Loads the CSV file if it exists, or returns an empty list if the file doesn't exist."""
    if os.path.exists(csv_file):
        int_fields = ["batch", "seed", "idim", "fdim", "stage", "k"]
        float_fields = ["sigma"]

        with open(csv_file, mode="r", newline="") as file:
            reader = csv.DictReader(file)
            data = []
            for row in reader:
                for field in int_fields:
                    if field in row and row[field] != "":
                        row[field] = int(row[field])
                for field in float_fields:
                    if field in row and row[field] != "":
                        row[field] = float(row[field])
                data.append(row)
            return data
    else:
        return []


def check_if_combination_exists(data, params):
    for entry in data:
        if (
            entry["sigma"] == params.sigma and
            entry["blend"] == params.blend and
            entry["fusion_method"] == params.fusion_method and
            entry["eps"] == params.eps and
            entry["seed"] == params.seed and
            entry["init_dim"] == params.init_dim and
            entry["stage_dim"] == params.stage_dim and
            entry["stages"] == params.stages and
            entry["k"] == params.k
        ):
            return True
    return False


def save_data_to_csv(data, filename):
    headers = data[0].keys() if data else []
    with open(filename, "w", newline="") as file:
        writer = csv.DictWriter(file, fieldnames=headers)
        writer.writeheader()
        writer.writerows(data)


def set_seed(seed):
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)
    np.random.seed(seed)
    random.seed(seed)
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False


def cosine_similarity(test_features, train_features, train_labels, test_labels):
    test_features = F.normalize(test_features, dim=-1)
    train_features = F.normalize(train_features, dim=-1)
    gamma_list = [i * 10000 / 5000 for i in range(5000)]
    best_gamma_acc, best_gamma = 0, 0
    for gamma in tqdm(gamma_list, leave=False):
        sim = test_features @ train_features.t()
        logits = torch.exp(-gamma * (1 - sim)) @ train_labels
        pred = logits.topk(1, dim=1, largest=True, sorted=True).indices
        correct = pred.squeeze().eq(test_labels.view(-1))
        acc = correct.float().mean().item() * 100
        if acc > best_gamma_acc:
            best_gamma_acc, best_gamma = acc, gamma
    return best_gamma_acc, best_gamma


def one_nn_classification(test_features, train_features, train_labels, test_labels):
    if train_labels.ndim > 1:
        train_labels = train_labels.argmax(dim=1)
    test_features = F.normalize(test_features, p=2, dim=1)
    train_features = F.normalize(train_features, p=2, dim=1)
    similarity = torch.mm(test_features, train_features.t())
    nearest_indices = similarity.argmax(dim=1)
    pred_labels = train_labels[nearest_indices]
    correct = pred_labels.eq(test_labels)
    accuracy = correct.float().mean().item() * 100.0
    return accuracy
